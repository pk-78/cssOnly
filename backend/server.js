...
app.post("/api/submit", upload.array("images"), async (req, res) => {
    try {
      ...
      if (!name || !forWhom || req.files.length < 5) {
        return res.status(400).json({ message: "All fields are required, and at least 5 images must be uploaded." });
      }
...
app.get("/api/submit/:id", async (req, res) => {
    try {
      ...
      res.status(200).json({ message: "Form data retrieved successfully", formData });
    } catch (error) {
